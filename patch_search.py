...
with open("src/pages/store/Search.tsx", "r") as f:
    content = f.read()
...
content = content.replace('focus:ring-indigo-600', 'focus:ring-slate-900')
...
content = content.replace('p-4 sm:p-8 max-w-7xl mx-auto w-full min-h-[60vh]', 'p-4 sm:p-8 max-w-7xl mx-auto w-full min-h-[60vh] bg-[#FAFAFA]')
content = content.replace('bg-white focus:ring-2 focus:ring-inset', 'bg-white border-2 border-transparent focus:border-slate-900 focus:ring-0')

# Update title
content = content.replace('text-3xl font-bold tracking-tight text-slate-900 mb-6', 'text-3xl font-extrabold tracking-tight text-slate-900 mb-6')
...
content = re.sub(
    r'<button\s*onClick={\(e\) => \{\s*e\.stopPropagation\(\);\s*addToCart\(product\);\s*\}}\s*className="w-full bg-slate-900 text-white font-bold py-3 rounded-xl flex justify-center items-center gap-2 hover:bg-indigo-600 transition-colors"\s*>',
...
with open("src/pages/store/Search.tsx", "w") as f:
    f.write(content)
