...
import { Plus, Edit, Trash2, Loader2, X, Image as ImageIcon, Camera, Search } from 'lucide-react';
...
export default function Categories() {
  ...
  const fetchCategories = async () => {
    try {
      ...
      const catsData = catSnapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data()
      })) as Category[];
  ...
  const handleDelete = async (id: string, name: string) => {
    try {
      ...
      let confirmMessage = `Delete this category ("${name}")?`;
      if (!snapshot.empty) {
        confirmMessage = `This category contains ${snapshot.size} products. Delete anyway and move them to Uncategorized?`;
      }

      if (window.confirm(confirmMessage)) {
        if (!snapshot.empty) {
          const updatePromises = snapshot.docs.map(productDoc => 
            updateDoc(doc(db, 'products', productDoc.id), { categoryId: null })
          );
        ...
        <div className="flex items-center gap-4">
          <div className="relative">
            ...
            <input 
              ...
              className="pl-9 pr-4 py-2 rounded-full border border-slate-200 shadow-sm focus:border-slate-900 focus:ring-slate-900 text-sm w-full md:w-64"
            />
          </div>
          <button
            onClick={openAddModal}
            className="flex items-center space-x-2 bg-slate-900 text-white px-5 py-2.5 rounded-full hover:bg-slate-800 transition-colors shadow-sm text-sm font-bold uppercase tracking-wider whitespace-nowrap"
          >
          ...
          {filteredCategories.map(category => (
            <div key={category.id} className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => handleDelete(category.id, category.name)} className="p-2 text-slate-400 hover:text-red-600 bg-slate-50 hover:bg-red-50 rounded-lg transition-colors text-sm font-semibold flex items-center gap-1">
